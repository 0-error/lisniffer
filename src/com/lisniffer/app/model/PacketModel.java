...
public class PacketModel implements Serializable{
	...
	private String packet_type ;						// 0-1字节，00 04是seng by us(4) 3G
	private int LinkLayer_address_type ;				// 2-3字节  3G
	private int LinkLayer_address_len ;					// 4-5字节  3G
	private String protocol1 ;							// 14-15字节或 12-13字节，08 00:IP ,08 06:ARP  WIFI数据包的协议字段
	private String mac_destination ;					// 0-5字节   WIFI
	private String mac_source;							// 6-11字节  WIFI
	...
	private String protocol2 = "Unknown";				// 9字节，协议，为ip数据包的协议字段
	...
	public String ShowInfo(){
		if(packet_type == null){   //packet_type为null，证明是WIFI包
			if(!protocol1.equals("IP")){ //wifi数据包协议字段不是ip,则tcp个udp没有协议字段
				return ShowDataHeader() + ShowLLHeaderWIFI() + ShowData() ;
			}else if(protocol2.equals("TCP")){ //tcp包时
			...
			}else if(protocol2.equals("UDP")){  //udp包时
