...
public class ReadFile {
	...
	public List<PacketModel> readFiletoArraylist(String fileName){
		...
		try {
			...
			readByteToString(20) ;							//跳过无用字节（Magic,4B;Major,2B;Minor,2B;ThisZone,4B;SigFigs,4B;SnapLen,4B。总共20字节）
			link_type = readByteToString(1) ;				//获取LinkType
			readByteToString(3) ;							//跳过无用字节
			while(true){
				index = 0 ;
				PacketModel packet = new PacketModel();
				if(total_index >= filesize)break;  //是否对文件读取完
				else{
					DataHeaderAnalysis(packet) ;
					if(LLHeaderAnalysis(packet)){
						IPPacketAnalysis(packet);
						if(packet.getProtocol2().equals("TCP")){  //判断是否为tcp包
							TCPPacketAnalysis(packet) ;
						}else if(packet.getProtocol2().equals("UDP")){  //判断是否为udp包
	...
	private String readByteToIPaddress() {
		...
		while(true){
			try {
				temp = in.read() ;   //System.in.read()可以实现输入字符，返回字符的Unicode码
				index++;
				total_index++;
				s.append(temp);
				c++;
	...
	private boolean LLHeaderAnalysis(PacketModel packet) {				//链路层头分析
		...
		}else if(link_type.equals("01 ")){   //1 Ethernet,and Linux loopback devices
	...
	private void DataHeaderAnalysis(PacketModel packet) {			//数据包头分析
		...
		long caplen = readByteToLong(4);                 //当前数据区的长度，即抓到的数据帧长度
		packet.setCaplen(caplen);
		
		long len = readByteToLong(4);                    //离线数据长度，网络中实际数据帧的长度
		packet.setLen(len) ;
	...
	public String readByteToString(int n){			//读 n个字节，返回一个字符串
		...
		try {
			...
			int c = 0 ;
			while(c<n){
				v[c] = tempbytes[c]&0xFF; //将byte[]转化十六进制的字符串
